...
| `GET /health`                                | `{"ok":true}`                                    |
| `POST /v1/instances`                         | the report and the port, 201 — or 409 at the cap |
...
A start body names the game, the session, the bundle and sim-config paths, and the bearer the child
presents to `@grove/game-manager`. It is a placement already decided upstream, so nothing here
weighs it — a box cannot answer which box should hold a session.
...
`GROVE_MANAGER_URL`, `GROVE_MANAGER_TOKEN`, `GROVE_HEAP_LIMIT_BYTES` and `GROVE_TICK_BUDGET_MS`.
Nothing the child needs is discovered by the child. The port is part of that: this box asks the
kernel which one is free and hands it over as `GROVE_BIND`, rather than keeping a range that would
drift from what is actually bound here. The child inherits none of this box's own environment.

## Local health

Each child is polled on its own `/healthz` over loopback, and what that reading says is what the
report carries — `starting`, `healthy`, `draining` or `unhealthy` — never a claim a child made about
itself. A probe that fails inside the boot grace leaves an instance `starting`; after it, unhealthy.
...
A ticker posts a `HostHeartbeat` to `@grove/server-manager`: this box, its region, its capacity, and
one `InstanceReport` per child it still holds a process for. Every instance every beat rather than a
delta, so a dropped beat costs nothing to recover, and a reaped child is absent rather than reported
dead. A failed beat is logged and dropped — the next one carries the whole state, and a queue of
stale beats would describe a box as it was.

`cpuLoad` and `memoryFreeBytes` come from `/proc`, normalized by core count so one number compares
two boxes of different sizes. Both are zero where the kernel publishes neither: the fleet's hard
bound on a box is its instance cap, and these two only break ties.
...
| Seam                  | Owns                                              |
| --------------------- | ------------------------------------------------- |
| `supervisor.Launcher` | what forks a process, and the environment it gets |
| `supervisor.Prober`   | what asks a child how it is                       |
| `supervisor.Ports`    | which port a child binds                          |
...
| `SERVER_MANAGER_URL`    | where the beat goes                                               |
...
| `GAME_TOKEN_SECRET`     | handed to every child, which verifies join tickets with it        |
...
and a redeploy of this agent must not end anyone's session.
