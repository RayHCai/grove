...
// store is written THROUGH — a saved host is readable immediately, and the trip to disk is a
...
export class PersistedState implements PersistedSource {
    ...
    /**
     * Captures `record` into the cache now and writes it through to the store.
     *
    ...
    save(record: HostRecord): Promise<void> {
        ...
        return this.#kv.set(PERSISTENCE_SCOPE, record.hostId, fields);
    }
