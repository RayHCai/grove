...
describe('destroy (§6)', () => {
    it('flips alive immediately but defers teardown to the drain', () => {
        ...
        expect(rt.gameInstance!.find({ tag: 'box' }).length).toBe(1);

        rt.entityManager.drainDestroyed();
        expect(rt.gameInstance!.find({ tag: 'box' }).length).toBe(0); // torn down
    });

    it('cascades to children — the whole subtree flips alive false', () => {
        const rt = loadGame();
        const parent = rt.gameInstance!.spawn('crate', 0, 0);
        const child = rt.gameInstance!.spawn('crate', 0, 0);
        const grandchild = rt.gameInstance!.spawn('crate', 0, 0);
...
describe('getTouching (§5.4)', () => {
    it('returns an empty array (never null) for an entity with no collider', () => {
        ...
        rt.gameInstance!.spawn('crate', 0, 0).tag('enemy');
    ...
    it('excludes self and reports overlap once a collider is present', () => {
        const rt = loadGame({ bounds: bounds(-100, 100, 100, -100) });
        const a = rt.gameInstance!.spawn('crate', 0, 0);
        const b = rt.gameInstance!.spawn('crate', 0, 0).tag('enemy');
