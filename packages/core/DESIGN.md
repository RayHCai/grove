...
| `config.ts`    | `DEFAULT_SIM_RATE`/`DEFAULT_SEND_RATE` (60/20), `BREAKER_THRESHOLD` 100, `MAX_SEND_DEPTH` 64, `MAX_DEDUP_KEYS` 1024, `MAX_LOG_RECORDS` 512, `MAX_REWIND_MS` 250, `MAX_BUBBLE_LENGTH` 200, `MAX_ENTITIES` 10000, `MAX_HANDLER_MS` 50, `DEFAULT_PRNG_SEED` 1, `resolveConfig`                                                                                                                                                                                                                                                                                                        |
...
imports core, so the edge arrives as a function. `LoadOptions` also carries the host's `log` sink — core
writes to no console and holds no transport, so without one every `warn` the engine makes has no reader —
and the PRNG `seed`, defaulting to `DEFAULT_PRNG_SEED`, which is what a server and its mirrors must be given
in common to draw one stream. `leavePlayer` goes innermost host outward and removes last — `@onEnd` at
