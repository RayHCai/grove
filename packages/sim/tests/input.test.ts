...
function onAvatar<T>(h: Harness, playerId: string, klass: new () => object, name: string): T {
    ...
    return [...h.sim.runtime.instances.forHost(host)].find((i) => i.className === name)
        ?.instance as T;
}
...
describe('ackSeq is the highest contiguous RESOLVED seq', () => {
    ...
    it('refuses a seq further ahead than the window could ever apply', () => {
        ...
        expect(h.sim.sessions[0]?.admission.highestSeen).toBe(-1);
    });
});
...
describe('the phases, synthesized from edges alone', () => {
    ...
    it('fills movement intent from the panel-mapped move axes', () => {
        const h = harness({ config: { gameScripts: [Rules] } });
        const peer = h.joined('a');
        const player = h.sim.runtime.playerManager?.byId('c1');
