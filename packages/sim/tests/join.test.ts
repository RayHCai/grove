...
import { JOIN_DEADLINE_MS, MAX_UNJOINED_CONNECTIONS, joinDeadlineTicks } from '../src/constants.js';
...
describe('the client speaks first', () => {
    it('accept() mints a connection, sends nothing, and mutates no roster', () => {
        ...
        expect(h.sim.sessions).toHaveLength(1);
        expect(h.sim.sessions[0]?.player).toBeNull();
    });
    ...
    it('sanitizes the untrusted name, and the roster carries the sanitized form', () => {
        ...
        const name = h.sim.runtime.playerManager?.players[0]?.name ?? '';
    ...
    it('a blank name becomes a placeholder rather than an empty roster entry', () => {
        ...
        peer.join('   ');
        h.pumpTicks(6);
        expect(h.sim.runtime.playerManager?.players[0]?.name).toBe('player');
    });
});
...
describe('refusals are envelopes, never a bare close', () => {
    it('maxPlayers refuses with Reject { full } and leaves the roster untouched', () => {
        ...
        expect(h.sim.runtime.playerManager?.players).toHaveLength(2);
    });
...
describe('the join snapshot is a walk of the live world', () => {
    ...
    it('omits a destroyed entity, because liveIds() is the source', () => {
        const h = harness({ config: { gameScripts: [Rules] } });
        h.joined('a');
        const doomed = h.sim.runtime.entityManager.spawn('ghost', 1, 1);
        doomed.destroy();
...
describe('the unjoined connection is bounded', () => {
    ...
    it('counts the deadline in ticks, so a world falling behind does not evict its joiners', () => {
        const h = harness();
        h.connect();
        h.pumpTicks(2);
        // One tick short: the sweep runs every tick, so an off-by-one here is a peer closed a whole
        // tick early on every join.
        h.pumpTicks(joinDeadlineTicks(h.sim.config.simRate) - 3);
        expect(h.sim.sessions).toHaveLength(1);
...
describe('disconnection', () => {
    it('close removes the connection and the player, and the next broadcast skips it', () => {
        ...
        expect(h.sim.sessions.map((c) => c.connectionId)).toStrictEqual(['c2']);
        expect(h.sim.runtime.playerManager?.players.map((p) => p.id)).toStrictEqual(['c2']);
        second.clear();
        h.pumpTicks(4);
    ...
    it('fires @onPlayerLeave on the Game-hosted ServerScript', () => {
        ...
        const rules = [...h.sim.runtime.instances.forHost('game')][0]?.instance as Rules;
    ...
    it('orders the host to close a session it released itself', () => {
        const h = harness({ config: { gameScripts: [Rules] } });
        h.joined('a');
        h.connect();
