...
describe('a template first used mid-session reaches the clients already connected', () => {
    ...
    it('sends it AHEAD of the state envelope that first spawns one', () => {
        ...
        h.sim.declareVisuals(coin);
        h.sim.runtime.entityManager.spawn('coin', 0, 0);
        h.pumpTicks(6);
    ...
    it('defines the asset on core’s table too, so `assets.get` answers for it', () => {
        const h = harness({ config: { gameScripts: [Rules] } });
        h.joined('a');
        expect(h.sim.runtime.assets?.get('coin')).toBeNull();

        h.sim.declareVisuals(coin);
        expect(h.sim.runtime.assets?.get('coin')?.key).toBe('coin');
    });
});
