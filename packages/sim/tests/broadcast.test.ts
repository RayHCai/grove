...
import { drainOnce } from '../src/replicate.js';
...
describe('the drain runs once and the broadcast fans it out', () => {
    it('a single spawn appears in EVERY connection’s envelope', () => {
        ...
        h.sim.runtime.entityManager.spawn('crate', 1, 2);
        h.pumpTicks(6);
    ...
    it('carries the whole transform, eight keys, in core’s field order', () => {
        const h = harness({ config: { gameScripts: [Rules] } });
        const peer = h.joined('a');
        const crate = h.sim.runtime.entityManager.spawn('crate', 5, 6);
        crate.layer = 2;
        crate.setScale(3);
        peer.clear();
        h.pumpTicks(6);
    ...
    it('drops a spawn-then-destroy pair rather than shipping an unreadable snapshot', () => {
        ...
        const doomed = h.sim.runtime.entityManager.spawn('ghost', 0, 0);
...
describe('state is addressed by host and scoped per player', () => {
    ...
    it('addresses an entity host by netId', () => {
        ...
        const health = [...h.sim.runtime.instances.forHost(host)][0]?.instance as Health;
        health.health = 1;
        h.pumpTicks(6);
...
describe('wrapper state crosses as its wire form, not as a class', () => {
    ...
    it('persists a departing player’s wrapper state at the leave, through the KV store', async () => {
        ...
        // Ordered as its WIRE form, not as the class: the store holds JSON, so a save that captured
        // the instance would write a record no rejoin could read back.
        expect(h.saves).toStrictEqual([
            {
                hostKey: 'player:alice',
    ...
    it('counts a mark whose host died first as churn, and not as a bug report', () => {
        ...
        expect(h.sim.staleMarks).toBe(1);
        expect(h.sim.droppedMarks).toBe(0);
...
describe('the fan-out', () => {
    it('addresses the transform envelope to every peer at once, so a host can encode it once', () => {
        const h = harness({ config: { gameScripts: [Rules] } });
        const peers = [h.joined('a'), h.joined('b'), h.joined('c')];
        expect(peers).toHaveLength(3);

        h.alignToSend();
        const sends = h.lastOutput!.sends;
        // One send for the whole broadcast whatever the peer count, and one per peer for the state
        // envelope: after the envelope split the transform frame is the whole of the shared subset,
        // and this shape is what lets a host pay one encode for it.
        const transforms = sends.filter((s) => s.envelope.kind === 'transform');
        expect(transforms).toHaveLength(1);
        expect(transforms[0]?.to).toHaveLength(3);
        expect(transforms[0]?.class).toBe('droppable');
        expect(sends.filter((s) => s.envelope.kind === 'state')).toHaveLength(3);
    });
...
describe('a template instantiation crosses as one group', () => {
    ...
    it('sends one group op holding the whole subtree, in journal order', () => {
        ...
        h.sim.runtime.gameInstance!.spawn('turret', 10, 20);
        h.pumpTicks(6);
    ...
    it('names the attached script by its id, with no class name anywhere on the wire', () => {
        ...
        const peer = h.joined('a');
        h.settle([peer]);
        h.sim.runtime.gameInstance!.spawn('turret', 0, 0);
        h.pumpTicks(6);
...
describe('a joiner is not told twice about what its snapshot already holds', () => {
    it('receives no structural op that predates its own snapshot', () => {
        ...
        h.alignToSend();
        const crate = h.sim.runtime.entityManager.spawn('crate', 3, 4);
        const netId = crate.entityId as unknown as number;
        h.pumpTicks(1);
