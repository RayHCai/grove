...
/**
 * Why an authority turned a join away. Wider than the wire's `RejectReason`: a ticket is checked
 * before the upgrade, so its refusal is an HTTP status that never becomes a reject envelope.
 */
...
export interface GamePlayerProps {
    ...
    serverUrl: string;
    /** The short-lived, session-scoped credential the allocator minted, not a platform session. */
    ticket: string;
    onReady?: () => void;
 ...
 * It holds no authority and checks nothing: every boundary that matters is the authority's.
 */
